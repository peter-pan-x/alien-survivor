  ...
  if (player.exp >= expNeeded) {
    player.exp -= expNeeded;
      ...
      if (now - lastDamageTimeRef.current > GAME_CONFIG.PLAYER.DAMAGE_COOLDOWN) {
        const playerCollisionRadius = player.radius * (GAME_CONFIG.COLLISION?.PLAYER_VS_ENEMY_PLAYER_RADIUS_MULTIPLIER ?? 0.7);
          ...
          <div className="mb-2 w-full max-w-[600px] space-y-1">
            <div className="flex items-center gap-1 text-lg select-none">
              {Array.from({ length: player.maxLives }).map((_, i) => (
                <span
                  key={i}
                  className={i < (player.lives ?? 1) ? "text-red-500" : "text-slate-500"}
                >
                  ❤
                </span>
              ))}
              ...
              {GAME_CONFIG.RENDERING.SHOW_HEALTH_BARS && (
                <div className="flex-1">
                  <div className="text-xs text-white mb-1 font-semibold">
                    生命值 {Math.max(0, Math.floor(player.health))}/{player.maxHealth}
                  </div>
