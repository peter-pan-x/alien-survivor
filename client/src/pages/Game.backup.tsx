...
const SKILLS: Skill[] = [
  ...
  { id: "life_steal", name: "生命汲取", description: "击杀敌人恢复1点生命（可重复选择，每次+1）", type: "special" },
  ...
  useEffect(() => {
    ...
    const gameLoop = () => {
      ...
      if (now - lastShotTimeRef.current > shootInterval) {
        ...
        const bulletSpeed = 8;
        const spreadAngle = player.bulletCount > 1 ? GAME_CONFIG.BULLET.SPREAD_ANGLE : 0;
          ...
          if (distance < bullet.radius + enemy.radius) {
            ...
            if (enemy.health <= 0) {
              ...
              if (acquiredSkills.includes("life_steal")) {
                const healAmount = player.lifeStealAmount ?? 1;
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-2 sm:p-4">
      ...
      {gameState === "playing" && (
          ...
          <div className="mb-2 w-full max-w-[600px] space-y-1">
            <div className="flex items-center gap-1 text-lg select-none">
              {Array.from({ length: player.maxLives }).map((_, i) => (
                <span key={i} className={i < (player.lives ?? 1) ? "text-red-500" : "text-slate-500"}>❤</span>
              ))}
            </div>
            <div className="flex gap-2">
              {GAME_CONFIG.RENDERING.SHOW_HEALTH_BARS && (
        ...
        经验值 {player.exp} / {Math.ceil((GAME_CONFIG.LEVELING.EXP_PER_KILL * (GAME_CONFIG.LEVELING.BASE_KILLS_FOR_FIRST_LEVEL ?? 5)) * Math.pow((GAME_CONFIG.LEVELING.GROWTH_RATE ?? 1.33), Math.max(0, player.level - 1)))}
              </div>
            ...
            style={{ width: `${(player.exp / Math.ceil((GAME_CONFIG.LEVELING.EXP_PER_KILL * (GAME_CONFIG.LEVELING.BASE_KILLS_FOR_FIRST_LEVEL ?? 5)) * Math.pow((GAME_CONFIG.LEVELING.GROWTH_RATE ?? 1.33), Math.max(0, player.level - 1)))) * 100}%` }}
                />
              </div>
            </div>
          </div>
